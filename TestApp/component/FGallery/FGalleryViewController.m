...
@synthesize isThumbViewShowing = _isThumbViewShowing;
...
//    if (_isThumbViewShowing && _useThumbnailView) {
...
//                                                                 self.navigationController.navigationBar.center.y + 20);
	_isActive = NO;
    _isShowDialog = NO;
...
{
    if (!_hideTitle){
        self.title = [NSString stringWithFormat:@"%i %@ %i", _currentIndex+1, NSLocalizedString(@"of", @"") , [_photoSource numberOfPhotosForPhotoGallery:self]];
    }else{
        [self setTitle:@""];
    }
}
...
- (void)galleryPhoto:(FGalleryPhoto*)photo loadFullsizeFail:(NSError *)error
{
    NSLog(@"Error : %@",error);
    if (![self isConnectionAvailable]) {
        //        [self showAlertMessage:@"Memorial" withMessage:kCannotConnectInternet];
        if(!_isShowDialog){
            _isShowDialog = YES;
...
//    [self showAlertMessage:@"Memorial" withMessage:kRequestTimeoutMessage];
}
